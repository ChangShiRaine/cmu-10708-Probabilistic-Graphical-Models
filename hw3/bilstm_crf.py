...
class BiLSTM_CRF(nn.Module):
    ...
    def log_partition(self, feature_seq):
        ...
        alphas = singleton_score[0, torch.arange(batch), :]  # [batch, state]
        alphas += self.start_score.unsqueeze(0) 
 
        for t in range(1, seq):
            alpha_t = []
            for tag in range(self.state_size):
                e_scores = singleton_score[t, :, tag] # [batch]

                t_scores = self.pairwise_score[:, tag] # [state]
                t_scores = t_scores.unsqueeze(0) # [1, state]
                scores = t_scores + alphas

                alpha_t.append(e_scores + torch.logsumexp(scores, dim=1))

            new_alphas = torch.stack(alpha_t).t()
            alphas = new_alphas

        end_scores = alphas + self.stop_score.unsqueeze(0) 
    ...
    def viterbi_decode(self, feature_seq):
        ...
        for t in range(1, seq):
            ...
            for tag in range(self.state_size):
                ...
                alpha_t.append(e_scores + max_score)
        ...
        max_score = max_final_scores
        state_seq = [] # [seq, batch]

        for i in range(batch):
            sample_final_tag = max_final_tags[i].item()
            sample_backpointers = backpointers[: seq - 1]
            sample_path = self.find_best_path(i, sample_final_tag, sample_backpointers)
            state_seq.append(sample_path)

        state_seq = torch.LongTensor(state_seq).t()
